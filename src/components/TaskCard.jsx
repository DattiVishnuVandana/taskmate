...
const TaskCard = ({task,handleDel,xyz}) => {
  return (
    <div>
           <li
 className={`list-row p-2 shadow-md ${
   task.completed ? "shadow-green-500" : "shadow-red-500"
 }`}
 key={task.id}
>

    <div className="text-4xl font-thin opacity-30 tabular-nums ">{task.id}</div>
    <div className="text-xs uppercase font-semibold opacity-60">-{xyz}-{task.name}</div>
     
     <button className="btn btn-square btn-ghost" onClick={()=>handleDel(task.id)}>
